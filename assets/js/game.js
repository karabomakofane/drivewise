/* =============================================
   ...
   - Phone controller via WebSocket
   ============================================= */
...
/* =============================================
   PHONE CONTROLLER — WebSocket client
   The game connects as 'game' role.
   The phone connects as 'controller' role.
   server.js routes messages between them.
   ============================================= */
let gameWS           = null;
...
function connectGameWS() {
  try {
    ...
    gameWS = new WebSocket(`${proto}//${location.host}`);

    gameWS.onopen = () => {
      gameWS.send(JSON.stringify({ type: 'identify', role: 'game' }));
    ...
    gameWS.onmessage = (e) => {
      try {
        ...
        if (msg.type === 'controller_connected') {
          ...
          showFB('📱 Phone controller connected!', 'info');
          updateControllerBadge(true);
        }

        /* Phone controller disconnected */
        if (msg.type === 'controller_disconnected') {
          controllerActive = false;
          showFB('📱 Phone controller disconnected', 'bad');
          updateControllerBadge(false);
        }

        /* Phone tilt → move player lane */
        if (msg.type === 'tilt' && G.running) {
          const targetLane = msg.lane;
          if (targetLane !== P.lane) {
            P.lane = Math.max(0, Math.min(NUM_LANES - 1, targetLane));
          }
        }

        /* Phone brake button or forward tilt → brake */
        if (msg.type === 'brake' && G.running) {
          P.braking = msg.braking;
        }
...
/* Send signal to phone — crash vibration or wrong buzz */
function notifyPhone(type) {
  if (gameWS && gameWS.readyState === WebSocket.OPEN) {
    gameWS.send(JSON.stringify({ type }));
  }
}
...
function updateRain() {
  ...
  if(Math.random() < 0.4) RAIN.push({
    x: Math.random()*canvas.width, y:-10,
    spd: 8+Math.random()*6, len: 10+Math.random()*8,
...
function drawHeadlights() {
  ...
  cone.addColorStop(.4,'rgba(255,250,220,.08)');
  cone.addColorStop(1,'rgba(255,250,220,0)');
  ctx.fillStyle = cone;
  ctx.beginPath();
  ctx.moveTo(P.x-PW/2,P.y); ctx.lineTo(P.x-80,P.y-200);
  ctx.lineTo(P.x+80,P.y-200); ctx.lineTo(P.x+PW/2,P.y);
  ...
  if(target !== G.envIndex && !G.transitioning) {
    ...
    const tick = () => {
      ...
      if(G.envTransition >= 1) {
        G.envTransition = 0; G.envIndex = target; G.transitioning = false;
...
function drawRoad() {
  ...
  if(curr.rain || (t>.3 && next.rain)) {
    ...
    wg.addColorStop(.5,`rgba(88,166,255,${wa})`);
...
function drawPlayer() {
  ...
  /* Phone controller indicator on car */
  if(controllerActive) {
    ctx.fillStyle = '#3fb950';
    ctx.font = '10px serif'; ctx.textAlign = 'center';
    ctx.fillText('📱', P.x, py - 8);
    ctx.textAlign = 'left';
  }
}
...
function drawOb(ob) {
  ...
  if(ob.isBoss) { ctx.shadowColor = '#ff1744'; ctx.shadowBlur = 20; }
...
/* =============================================
   WARNING FLASH
   Slides down 2 seconds before obstacle hits
   ============================================= */
...
function drawWarningFlash() {
  ...
  ctx.fillStyle = warn.color; ctx.textAlign = 'left';
...
function triggerBossWave() {
  ...
  document.getElementById('gameCanvas').classList.add('boss-active');

  const shuffled = [...GAME_SIGNS].sort(() => Math.random() - .5);
  [0, 1, 2].forEach((lane, i) => {
    const sign = shuffled[i % shuffled.length];
    const cfg  = DIFF[G.difficulty];
    OBS.push({
      lane, x: lc(lane), y: -100 - i * 80,
      w: 38, h: 60,
      spd:     cfg.spd + (G.level-1)*.4 + 1.5,
  ...
  setTimeout(() => {
    G.isBossWave = false;
    document.getElementById('gameCanvas').classList.remove('boss-active');
...
function drawHUD() {
  ...
  if(controllerActive) {
    ctx.fillStyle = 'rgba(63,185,80,.2)';
    ctx.beginPath(); ctx.roundRect(8, G.isBossWave?76:44, 110, 22, 6); ctx.fill();
    ctx.font = 'bold 10px sans-serif'; ctx.fillStyle = '#3fb950';
    ctx.fillText('📱 PHONE CONNECTED', 14, G.isBossWave?91:59);
  }
  ...
  if(bossProgress > 0 && !G.isBossWave) {
    ctx.fillStyle = 'rgba(0,0,0,.3)';
    ctx.fillRect(ROAD_X, canvas.height-14, ROAD_W, 4);
...
function drawIdle() {
  ...
  ctx.fillRect(0,canvas.height*.35,canvas.width,canvas.height);
  ...
  ctx.beginPath(); ctx.roundRect(canvas.width/2-155,canvas.height/2-95,310,165,14); ctx.fill();

  ctx.font = 'bold 28px Barlow Condensed,sans-serif';
  ctx.fillStyle = '#f5a623'; ctx.textAlign = 'center';
  ctx.fillText('DRIVEWISE', canvas.width/2, canvas.height/2-52);

  ctx.font = '12px DM Sans,sans-serif'; ctx.fillStyle = 'rgba(255,255,255,.65)';
  ctx.fillText('React to signs · Level up · Change worlds', canvas.width/2, canvas.height/2-28);
  ...
  ctx.fillText('👹 Boss waves every 5 levels', canvas.width/2, canvas.height/2+12);

  ctx.fillStyle = 'rgba(63,185,80,.8)';
  ctx.fillText('📱 Scan QR to use phone as controller', canvas.width/2, canvas.height/2+30);

  ctx.fillStyle = 'rgba(245,166,35,.9)';
  ctx.fillText('▶  Press START to play', canvas.width/2, canvas.height/2+52);
