/* =============================================
   game.js
   Canvas game engine — drawing, physics,
   obstacles, collision detection, controls
   ============================================= */
...
/* Get the X centre of lane index 0, 1 or 2 */
...
/* Live arrays */
let OBS   = [];  // obstacle objects + floating score text
let PARTS = [];  // crash particles
let roadOff = 0; // road animation scroll offset
let animId;      // current requestAnimationFrame id
...
function spawnOb() {
  ...
  const behaviors = ['straight','straight','zigzag','lane-switch','speed-change'];
  OBS.push({
    lane, x: lc(lane), y: -100, w: 38, h: 60,
    spd:     cfg.spd + (G.level - 1) * 0.4 + Math.random() * 0.8,
    baseSpd: cfg.spd + (G.level - 1) * 0.4,
    sign,
    beh:  behaviors[Math.floor(Math.random() * behaviors.length)],
    zDir: 1, zT: 0,      // zigzag state
    shown:    null,      // timestamp when sign entered view (for reaction timing)
    reacted:  false,     // whether player has already reacted to this sign
...
function drawHUD() {
  ...
  ctx.textAlign = 'center'; ctx.fillText(`LVL ${G.level}`, canvas.width / 2, 22);
  ctx.textAlign = 'left';
}
...
function updateObs() {
  ...
  for (let i = OBS.length - 1; i >= 0; i--) {
    ...
    /* Record when sign enters player's view */
    if (ob.y > -60 && !ob.shown) ob.shown = Date.now();

    /* Reaction window — score or penalise the player */
    ...
    /* Collision detection — triggers crash */
    if (ob.y + ob.h > P.y && ob.y < P.y + PH) {
      ...
      if (px2 > ox1 + 6 && px1 < ox2 - 6) {
        G.crashSign = ob.sign;
        crashFX(P.x, P.y);
        showFB('💥 CRASH! Preparing your quiz…', 'bad');
        G.running = false;
...
/* ---- PLAYER MOVEMENT + HUD SYNC ---- */
function updatePlayer() {
  P.x += (lc(P.lane) - P.x) * 0.15;
  const nl = Math.floor(G.score / 200) + 1;
  if (nl > G.level) { G.level = nl; showFB(`🚀 Level ${G.level}!`, 'info'); }
...
function drawIdle() {
  ...
  ctx.beginPath(); ctx.roundRect(canvas.width / 2 - 135, canvas.height / 2 - 70, 270, 108, 14); ctx.fill();
  ctx.font = 'bold 28px Barlow Condensed, sans-serif'; ctx.fillStyle = '#f5a623';
  ctx.textAlign = 'center'; ctx.fillText('DRIVEWISE', canvas.width / 2, canvas.height / 2 - 30);
  ctx.font = '13px DM Sans, sans-serif'; ctx.fillStyle = 'rgba(255,255,255,.6)';
  ctx.fillText('React to signs · Crash = Quiz', canvas.width / 2, canvas.height / 2 - 6);
  ctx.font = '12px DM Sans, sans-serif'; ctx.fillStyle = 'rgba(255,255,255,.35)';
  ctx.fillText('Personalised lesson on every crash', canvas.width / 2, canvas.height / 2 + 16);
  ctx.fillStyle = 'rgba(245,166,35,.7)';
  ctx.fillText('Press START to play', canvas.width / 2, canvas.height / 2 + 38);
  ctx.textAlign = 'left';
}