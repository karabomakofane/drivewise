/* =============================================
   coach.js — AI Coach, Leaderboard, Dashboard
   ============================================= */
...
function calcGrade(score){
  if(score>=500)return'A'; if(score>=300)return'B'; if(score>=150)return'C'; return'D';
}
...
function renderLeaderboard(){
  ...
  if(!el)return;
  const lb=getLeaderboard();
  if(lb.length===0){el.innerHTML='<div class="no-data-msg">No scores yet — play your first game!</div>';return;}
  const medals=['🥇','🥈','🥉'],mCls=['gold','silver','bronze'];
  const gCol={A:'var(--green)',B:'var(--blue)',C:'var(--accent)',D:'var(--accent2)'};
  el.innerHTML=lb.slice(0,5).map((e,i)=>`
    <div class="lb-row">
      ...
      <div class="lb-grade" style="color:${gCol[e.grade]};background:${gCol[e.grade]}22;border:1px solid ${gCol[e.grade]}44">${e.grade}</div>
    </div>`).join('');
}

/* ---- PERFORMANCE HISTORY ---- */
function getPerfHistory(){try{return JSON.parse(localStorage.getItem(PERF_KEY))||[];}catch{return[];}}
function savePerformance(){
  const hist=getPerfHistory();
  hist.push({score:G.score,grade:calcGrade(G.score),avgReact:G.avgReact,level:G.level,mistakes:G.mistakes,wrongSigns:G.wrongSigns.map(s=>s.id),ts:Date.now()});
...
function renderDashboard(){
  ...
  if(!el)return;
  if(hist.length===0){
    el.innerHTML='<div class="dash-card" style="grid-column:1/-1"><div class="no-data-msg">🎮 Play at least one game to unlock your dashboard.</div></div>';
    return;
  }
  ...
  const avgRT=rtGames.length?Math.round(rtGames.reduce((a,b)=>a+b.avgReact,0)/rtGames.length):0;
  const failCount={};
  ...
  el.innerHTML=`
    <div class="dash-card">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;margin-top:1rem">
        <div><div style="font-family:var(--font-head);font-size:1.4rem;font-weight:800;color:var(--blue)">${totalGames}</div><div style="font-size:.75rem;color:var(--muted)">Games played</div></div>
        <div><div style="font-family:var(--font-head);font-size:1.4rem;font-weight:800;color:var(--green)">${avgScore}</div><div style="font-size:.75rem;color:var(--muted)">Avg score</div></div>
      </div>
    </div>
    <div class="dash-card">
      ...
      ${rtHist.length>1?`
        <div class="rt-history">
          ${rtHist.map(rt=>{const h=Math.round((rt/maxRT)*56)+4;const cls=rt<400?'fast':rt>700?'slow':'';return`<div class="rt-bar ${cls}" style="height:${h}px" title="${rt}ms"></div>`;}).join('')}
        </div>
        <div style="display:flex;justify-content:space-between;margin-top:.4rem;font-size:.68rem;color:var(--muted)">
          <span>← Older</span><span style="color:var(--green)">■ Fast &lt;400</span><span style="color:var(--accent2)">■ Slow &gt;700</span><span>Recent →</span>
        </div>
        <div style="margin-top:.75rem"><span style="font-family:var(--font-head);font-size:1.8rem;font-weight:800;color:${avgRT<500?'var(--green)':avgRT<700?'var(--accent)':'var(--accent2)'}">${avgRT}ms</span><span style="font-size:.8rem;color:var(--muted);margin-left:.4rem">average</span></div>
        <div style="font-size:.78rem;color:var(--muted);margin-top:.25rem">Target: <strong style="color:var(--green)">under 500ms</strong> ${avgRT<500?'✅ You\'re there!':avgRT<700?'— Getting closer!':'— Needs work'}</div>
    ...
    <div class="dash-card">
      ...
      <div class="sign-performance-grid">
        ${GAME_SIGNS.map(s=>{
          ...
          return`<div class="sign-perf-row"><span class="sp-icon">${s.emoji}</span><span class="sp-label">${s.label}</span><div class="sp-bar-wrap"><div class="sp-bar-fill ${cls}" style="width:${barPct}%"></div></div><span class="sp-pct" style="color:${pct===0?'var(--green)':pct<40?'var(--accent)':'var(--accent2)'}">${fails>0?fails+'x':'✓'}</span></div>`;
        }).join('')}
      </div>
      <div style="font-size:.72rem;color:var(--muted);margin-top:.75rem">Numbers = times you failed each sign type</div>
    </div>
    <div class="dash-card">
      <h3>📊 Grade Distribution</h3>
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:.5rem;margin-bottom:1rem">
        ${Object.entries(grades).map(([g,count])=>`<div style="background:${gCol[g]}18;border:1px solid ${gCol[g]}44;border-radius:var(--r);padding:.6rem;text-align:center"><div style="font-family:var(--font-head);font-size:1.6rem;font-weight:900;color:${gCol[g]}">${g}</div><div style="font-size:.72rem;color:var(--muted)">${count}×</div></div>`).join('')}
      </div>
      <div style="font-size:.82rem;color:var(--muted)">Most common: <strong style="color:${gCol[topGrade]}">${topGrade}</strong> ${topGrade==='A'?'— Outstanding! 🏆':topGrade==='B'?'— Great work! ⭐':topGrade==='C'?'— Keep practising 💪':'— Study the signs! 📚'}</div>
    </div>`;
}
...
Write personalised coaching feedback referencing these exact details.`;
  ...
  try{
    ...
    const text=data.content?.[0]?.text||`Good effort driver! Grade ${grade} with a score of ${G.score}. Keep practising!`;
    ...
    document.getElementById('coach-thinking').style.display='none';
    document.getElementById('coach-msg').textContent=`Grade ${grade}, driver — score of ${G.score}. ${wrongNames.length>0?'You struggled with '+wrongNames.join(' and ')+' signs. ':'Sign reactions were solid. '}${G.avgReact>700?'Your reaction time of '+G.avgReact+'ms needs improvement — aim under 500ms. ':''}Keep practising and you\'ll be road-ready!`;
    document.getElementById('coach-msg').classList.add('show');
  }
...
function closeCoach(){document.getElementById('coach-overlay').classList.remove('show');}

/* ---- LICENCE CARD ---- */
function showLicenceCard(){
  closeCoach();
  const grade=calcGrade(G.score),pass=grade!=='D';
  const gCol={A:'var(--green)',B:'var(--blue)',C:'var(--accent)',D:'var(--accent2)'};
  const gc=gCol[grade];
  const uid='DW-'+Date.now().toString(36).toUpperCase().slice(-6);
  const date=new Date().toLocaleDateString('en-ZA');
  const gradeEl=document.getElementById('lc-grade');
  gradeEl.textContent=grade; gradeEl.style.color=gc; gradeEl.style.borderColor=gc;
...
function closeLicence(){document.getElementById('licence-overlay').classList.remove('show');}

function shareCard(){
  const grade=calcGrade(G.score);
  const text=`🚗 DriveWise K53 Simulator Result\nGrade: ${grade} | Score: ${G.score} | Reaction: ${G.avgReact||'—'}ms | Level: ${G.level}\n\nCan you beat my score? #DriveWise #K53 #RoadSafety 🇿🇦`;
  if(navigator.share){navigator.share({title:'My DriveWise Result',text});}
