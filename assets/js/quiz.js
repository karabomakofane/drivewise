/* =============================================
   quiz.js
   Crash quiz engine — builds personalised quiz
   from player mistakes, renders questions,
   handles answers and shows results
   ============================================= */

let QZ = { qs: [], cur: 0, correct: 0, missed: [] };

/* Build an inline sign visual for quiz questions */
function mkSignVisual(sign, sz) {
  const s = sz || 72;
  if (sign.id === 'stop') {
    return `<div style="width:${s}px;height:${s}px;background:#c0392b;clip-path:polygon(30% 0%,70% 0%,100% 30%,100% 70%,70% 100%,30% 100%,0% 70%,0% 30%);display:flex;align-items:center;justify-content:center;font-size:${Math.round(s*.18)}px;font-weight:900;color:white;font-family:var(--font-head)">STOP</div>`;
  }
  if (['warning','pedestrian','railway','animals'].includes(sign.id)) {
    return `<div style="position:relative;width:${Math.round(s*1.1)}px;height:${s}px">
      <svg viewBox="0 0 76 68" width="${Math.round(s*1.1)}" height="${s}">
        <polygon points="38,4 74,64 2,64" fill="#e05c4b" stroke="white" stroke-width="3"/>
      </svg>
      <div style="position:absolute;top:${Math.round(s*.32)}px;left:50%;transform:translateX(-50%);font-size:${Math.round(s*.38)}px">${sign.emoji}</div>
    </div>`;
  }
...
/* -----------------------------------------------
   buildQuiz()
   Analyses G (game state) to determine which
   signs caused problems, then builds a targeted
   lesson + question set and shows the overlay
   ----------------------------------------------- */
function buildQuiz() {

  /* 1. Collect unique problem signs — in priority order */
  const seen = new Map();
  ...
  const focusSigns = [...seen.values()].slice(0, 3);

  /* 2. Build question pool (2 for primary, 1 for others) */
  const pool = [];
  focusSigns.forEach((fs, idx) => {
    const sd = GAME_SIGNS.find(s => s.id === fs.id);
    if (!sd) return;
    const count = idx === 0 ? 2 : 1;
    [...sd.questions].sort(() => Math.random() - 0.5).slice(0, count)
      .forEach(q => pool.push({ ...q, sign: sd }));
  });

  const finalQs = pool.slice(0, 4);
  if (finalQs.length < 2) {
    const sd = GAME_SIGNS.find(s => s.id === focusSigns[0].id);
    if (sd) {
  ...
  /* 3. Crash banner */
  const cs = G.crashSign || focusSigns[0];
  document.getElementById('cq-emoji').textContent = cs.emoji;
  document.getElementById('cq-name').textContent  = cs.label + ' Sign';

  /* 4. Lesson for primary sign */
  const ps = GAME_SIGNS.find(s => s.id === (G.crashSign?.id || focusSigns[0].id));
  if (ps) {
  ...
  if (G.avgReact > 700 || G.slowSigns.length > 0) {
    ...
    document.getElementById('rt-text').innerHTML =
      ...
      `Target: <strong>under 500ms</strong>. At 60 km/h, a ${G.avgReact}ms delay ` +
      `means travelling <strong>${dist} metres</strong> without responding — ` +
...
function renderQ() {
  ...
  document.getElementById('q-opts').innerHTML = q.opts.map((o, i) =>
    `<button class="quiz-opt" onclick="ansQ(${i})">
       <span style="width:22px;height:22px;border-radius:50%;background:var(--bg2);border:1px solid var(--border);display:inline-flex;align-items:center;justify-content:center;font-size:.72rem;font-weight:700;flex-shrink:0">${LTRS[i]}</span>
       ${o}
     </button>`).join('');
...
function showQResults() {
  ...
  document.getElementById('qr-sub').textContent   = pass
    ? `${pct}% correct — great job! You understand these signs.`
    : `${pct}% correct — review the lesson and play again. Knowledge saves lives.`;
